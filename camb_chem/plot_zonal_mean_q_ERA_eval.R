...
# ERA data
nc0 <- nc_open(paste(obs_dir, "ERA/data/erai_q_2000-2011_N48_P37.nc", sep="/"), readunlim=FALSE)

# extract/define variables
era.a  <- ncvar_get(nc0,"Q")
...
lat.era<- ncvar_get(nc0, "latitude")
lon.era<- ncvar_get(nc0, "longitude")
hgt    <- ncvar_get(nc0, "p")
z      <- seq(length(hgt))
...
#M ht     <- ncvar_get(nc1, trop.pres.code)
...
# ###################################################################################################################################
pdf(file=paste(out_dir,"/",mod1.name,"-ERA_Q_pd.pdf", sep=""),width=8,height=6,paper="special",onefile=TRUE,pointsize=12)
