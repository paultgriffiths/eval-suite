...
nc1 <- nc_open(mod1.datafile, readunlim=FALSE) 

# set the directory where the R scripts are saved
run.path <- getwd()

# function to test if variables exist
var_exists <- function( ncid, varname ) {
	if( class(ncid) != 'ncdf4' )
                stop(paste("Error, first argument must be the return value from a nc_open command!"))
        return( ! is.null( ncid$var[[varname]] ))
}
...
qid1 <- var_exists(nc1, clo.code)
qid2 <- var_exists(nc1, n2o.code)

mod1.type <- "CheST"           # assume default
if ( identical(FALSE,qid1) && identical(FALSE,qid2) ) mod1.type <- "CheT"   # Trop only
...
# Plot NO2 column vs OMI
 ...
 print("PLOT_TAU_CH4, TROP_O3_COL, TROP_NO2_COL not called")
...
# Plot Lightning NOx
mmid <- var_exists(nc1, lgt.em.code )
if ( identical(TRUE,mmid) ) {
